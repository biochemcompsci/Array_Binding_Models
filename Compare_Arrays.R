...
gprAnalysisDefFile <- '/Volumes/Macintosh HD/Users/mattg/Dropbox (Nextval)/HealthTell_MPG_PS/Research/Modeling/Experimental/Chagas/24NOV2015/Chagas_Panel_Chagas_Specificity-12_min_10X.xlsx'
...
for(currentAnalysisGroup in analysisGroups) {
  ...
  quantileSignalDensityOutputFile <- paste0(outputFile, '/Analysis_Group_', currentAnalysisGroup, '_Quantile_Signal_Density.csv')
  ...
  quantileSignalDensities <- matrix(data = NA, nrow = length(dataSetNames), ncol = length(quantileCutoffs))
  colnames(quantileSignalDensities) <- quantileCutoffs
  rownames(quantileSignalDensities) <- dataSetNames
...
#     
    for(currDataSet in dataSetNames) {
      ...
      quantileSignalDensities[dataSetIdx,as.character(currQuantile)] <- (mean(gprDataZScoreScaled[quantileIdxs,dataSetIdx]) / length(gprDataZScoreScaled[quantileIdxs,dataSetIdx]))
...
#    evenn(matLists=currQuantileBinaryMatrixNonZero, prop=TRUE, annot = TRUE, display=TRUE, pathRes=comparativeOutputDir, CompName=paste0(currQuantile, '_Quantile_Bin'))
  ...
  write.csv(quantileCounts, file = quantileCountsOutputFile, sep = "\t", col.names = TRUE, row.names = TRUE)
  write.csv(quantileSignalDensities, file = quantileSignalDensityOutputFile, sep = "\t", col.names = TRUE, row.names = TRUE)
